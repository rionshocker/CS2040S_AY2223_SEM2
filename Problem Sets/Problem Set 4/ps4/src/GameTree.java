...
public class GameTree {
    ...
    int findValue() {
        if (root == null) {
            ...
            findValueHelper(root, Player.ONE);
            return root.value;
        }
    }

    void findValueHelper(TreeNode root, Player player) {
        if (root.leaf) {
            return;
        } else {
            for (int i = 0; i < root.numChildren; i++) {
                TreeNode child = root.children[i];
                findValueHelper(child, other(player));
            }

            if (player == Player.ONE) {
                int max = Integer.MIN_VALUE;
                for (int i = 0; i < root.numChildren; i++) {
                    if (max < root.children[i].value) {
